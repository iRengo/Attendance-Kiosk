  ...
  useEffect(() => {
    ...
    const fetchTeacher = async () => {
      try {
        ...
        if (!mounted) return;
        const data = res.data || {};
        setTeacherDetected(data && data.status === "success" ? data : null);
      } catch (e) {
        // ignore
      }
    };
          ...
          <div className="relative bg-gray-800 text-white rounded-lg p-4 w-80 shadow-lg ring-1 ring-black/20 z-10">
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <p className="text-sm text-gray-300">for {teacherDetected ? teacherDetected.name : "teacher"}</p>
              </div>
              <button className="text-gray-300 hover:text-white" onClick={() => setShowClassModal(false)} aria-label="Close">✕</button>
            </div>
            <select className="w-full p-2 bg-gray-700 text-white border border-gray-600 rounded mb-3" value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}>
            ...
            {selectedClass ? (
              (() => {
                ...
                return (
                  <div className="mb-3">
                    <p className="text-sm font-medium text-white">{cls.subjectName || cls.name}</p>
                    <p className="text-xs text-gray-300">{`${cls.gradeLevel || ''} ${cls.section || ''}`.trim()}</p>
                  </div>
                );
              })()
            ) : null}
